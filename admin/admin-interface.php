...
class WP_PVC_Admin_Interface extends WP_PVC_Admin_UI
{
	...
	public function get_success_message( $message = '' ) {
		if ( trim( $message ) == '' ) $message = __( 'Settings successfully saved.' , 'pvc' ); 
	...
	public function get_error_message( $message = '' ) {
		if ( trim( $message ) == '' ) $message = __( 'Error: Settings can not save.' , 'pvc' ); 
	...
	public function get_reset_message( $message = '' ) {
		if ( trim( $message ) == '' ) $message = __( 'Settings successfully reseted.' , 'pvc' ); 
		return '<div class="updated" id=""><p>' . $message . '</p></div>';
	}
	...
	public function get_font_weights() {
		$font_weights = array (
			'300'				=> __( 'Thin', 'pvc' ),
			'300 italic'		=> __( 'Thin/Italic', 'pvc' ),
			'normal'			=> __( 'Normal', 'pvc' ),
			'italic'			=> __( 'Italic', 'pvc' ),
			'bold'				=> __( 'Bold', 'pvc' ),
			'bold italic'		=> __( 'Bold/Italic', 'pvc' ),
		);
	...
	public function get_border_styles() {
		$border_styles = array (
			'solid'				=> __( 'Solid', 'pvc' ),
			'double'			=> __( 'Double', 'pvc' ),
			'dashed'			=> __( 'Dashed', 'pvc' ),
			'dotted'			=> __( 'Dotted', 'pvc' ),
			'groove'			=> __( 'Groove', 'pvc' ),
			'ridge'				=> __( 'Ridge', 'pvc' ),
			'inset'				=> __( 'Inset', 'pvc' ),
			'outset'			=> __( 'Outset', 'pvc' ),
		);
	...
	public function admin_forms( $options, $form_key, $option_name = '', $form_messages = array() ) {
		...
		foreach ( $options as $value ) {
			...
			switch( $value['type'] ) {
				...
				case 'google_api_key':
					...
					if ( ! isset( $value['checked_label'] ) ) $value['checked_label'] = __( 'ON', 'pvc' );
					if ( ! isset( $value['unchecked_label'] ) ) $value['unchecked_label'] = __( 'OFF', 'pvc' );

					?><tr valign="top">
						<th scope="row" class="titledesc">
                        	<?php echo $tip; ?>
							<label for="<?php echo $this->google_api_key_option; ?>"><?php echo __( 'Google Fonts API', 'pvc' ); ?></label>
						</th>
						<td class="forminp forminp-onoff_checkbox forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							<input
								...
								/> <span class="description" style="margin-left:5px;"><?php echo __( 'ON to connect to Google Fonts API and have auto font updates direct from Google.', 'pvc' ); ?></span>

							<div>&nbsp;</div>
							<div class="a3rev-ui-google-api-key-container" style="<?php if( 1 != $google_api_key_enable ) { echo 'display: none;'; } ?>">
								<div class="a3rev-ui-google-api-key-description"><?php echo sprintf( __( "Enter your existing Google Fonts API Key below. Don't have a key? Visit <a href='%s' target='_blank'>Google Developer API</a> to create a key", 'pvc' ), 'https://developers.google.com/fonts/docs/developer_api#APIKey' ); ?></div>
								<div class="a3rev-ui-google-api-key-inside 
									...
									<input
		                                ...
		                                placeholder="<?php echo __( 'Google Fonts API Key', 'pvc' ); ?>"
										<?php echo implode( ' ', $custom_attributes ); ?>
										/>
									<p class="a3rev-ui-google-valid-key-message"><?php echo __( 'Your Google API Key is valid and automatic font updates are enabled.', 'pvc' ); ?></p>
									<p class="a3rev-ui-google-unvalid-key-message"><?php echo __( 'Please enter a valid Google API Key.', 'pvc' ); ?></p>
								</div>
							</div>
						</td>
					</tr><?php
				...
				case 'onoff_toggle_box' :
					...
					?><tr valign="top">
						<th scope="row" class="titledesc">
                        	<?php echo $tip; ?>
							<label for="<?php echo $this->toggle_box_open_option; ?>"><?php echo __( 'Open Box Display', 'pvc' ); ?></label>
						</th>
						<td class="forminp forminp-onoff_checkbox forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							<input
								...
								<?php echo implode( ' ', $custom_attributes ); ?>
								/> <span class="description" style="margin-left:5px;"><?php echo __( 'ON and each admin panel setting box OPEN | CLOSED position are saved each time changes are SAVED.', 'pvc' ); ?></span>
                        </td>
					</tr><?php
				break;
							...
							<fieldset>
								...
								if ( is_array( $value['onoff_options'] ) && count( $value['onoff_options'] ) > 0 ) {
									foreach ( $value['onoff_options'] as $i_option ) {
										if ( ! isset( $i_option['checked_label'] ) ) $i_option['checked_label'] = __( 'ON', 'pvc' );
										if ( ! isset( $i_option['unchecked_label'] ) ) $i_option['unchecked_label'] = __( 'OFF', 'pvc' );
				...
				case 'image_size' :
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							...
							<label><?php _e( 'Width', 'pvc' ); ?> <input name="<?php echo $name_attribute; ?>[width]" id="<?php echo $id_attribute; ?>-width" type="text" class="a3rev-ui-<?php echo sanitize_title( $value['type'] ) ?>-width" value="<?php echo $width; ?>" /></label>
	
							<label><?php _e( 'Height', 'pvc' ); ?> <input name="<?php echo $name_attribute; ?>[height]" id="<?php echo $id_attribute; ?>-height" type="text" class="a3rev-ui-<?php echo sanitize_title( $value['type'] ) ?>-height" value="<?php echo $height; ?>" /></label>
	
							<label><?php _e( 'Hard Crop', 'pvc' ); ?> <input name="<?php echo $name_attribute; ?>[crop]" id="<?php echo $id_attribute; ?>-crop" type="checkbox" class="a3rev-ui-<?php echo sanitize_title( $value['type'] ) ?>-crop" <?php echo $crop; ?> /></label>
						...
						<td class="forminp">
                           ...
                           <!-- Font Face -->
							<select
								...
								>
								<optgroup label="<?php _e( '-- Default Fonts --', 'pvc' ); ?>">
                                <?php
                                ...
                                <optgroup label="<?php _e( '-- Google Fonts --', 'pvc' ); ?>">
                                <?php
                           ...
                           <!-- Preview Button -->
                           <div class="a3rev-ui-typography-preview"><a href="#" class="a3rev-ui-typography-preview-button button submit-button" title="<?php _e( 'Preview your customized typography settings', 'pvc'); ?>"><span>&nbsp;</span></a></div>
						...
						<td class="forminp forminp-border_corner">
                           ...
                           <!-- Border Corner : Rounded or Square -->
								<input
                                    ...
                                    checked_label="<?php _e( 'Rounded', 'pvc' ); ?>"
                                    unchecked_label="<?php _e( 'Square', 'pvc' ); ?>"
                                    type="checkbox"
                                    value="rounded"
							...
							<!-- Border Rounded Value -->
								<div class="a3rev-ui-border-corner-value-container">
                                	<div class="a3rev-ui-border_corner-top_left">
                                        <span class="a3rev-ui-border_corner-span"><?php _e( 'Top Left Corner', 'pvc' ); ?></span>
                                    ...
                                    <div class="a3rev-ui-border_corner-top_right">
                                        <span class="a3rev-ui-border_corner-span"><?php _e( 'Top Right Corner', 'pvc' ); ?></span> 
                                        <div class="a3rev-ui-slide-container">
                                            <div class="a3rev-ui-slide-container-start">
                                                <div class="a3rev-ui-slide-container-end">
                                    ...
                                    <div class="a3rev-ui-border_corner-bottom_right">
                                        <span class="a3rev-ui-border_corner-span"><?php _e( 'Bottom Right Corner', 'pvc' ); ?></span> 
                                        <div class="a3rev-ui-slide-container">
                                            <div class="a3rev-ui-slide-container-start">
                                                <div class="a3rev-ui-slide-container-end">
                                    ...
                                    <div class="a3rev-ui-border_corner-bottom_left">
                                        <span class="a3rev-ui-border_corner-span"><?php _e( 'Bottom Left Corner', 'pvc' ); ?></span>
                                        <div class="a3rev-ui-slide-container"> 
                                            <div class="a3rev-ui-slide-container-start">
                                                <div class="a3rev-ui-slide-container-end">
				...
				case 'border_styles':
					...
					?><tr valign="top">
						...
						<td class="forminp">
                           ...
                           <!-- Preview Button -->
                           <div class="a3rev-ui-settings-preview"><a href="#" class="a3rev-ui-border-preview-button a3rev-ui-settings-preview-button button submit-button" title="<?php _e( 'Preview your customized border styles settings', 'pvc'); ?>"><span>&nbsp;</span></a></div>
                           </div>
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-box_shadow">
                            ...
                            <input
                                    ...
                                    checked_label="<?php _e( 'YES', 'pvc' ); ?>"
                                    unchecked_label="<?php _e( 'NO', 'pvc' ); ?>"
                                    type="checkbox"
                                    value="1"
							...
							<select
                                ...
                                data-placeholder="<?php _e( 'Horizontal Shadow', 'pvc' ); ?>"
								>
								<?php
									for ( $i = -20; $i <= 20; $i++ ) {
										?>
							...
							<select
                                ...
                                data-placeholder="<?php _e( 'Vertical Shadow', 'pvc' ); ?>"
								>
								<?php
									for ( $i = -20; $i <= 20; $i++ ) {
										?>
                           ...
                           <!-- Box Blur Distance -->
							<select
                                ...
                                data-placeholder="<?php _e( 'Blur Distance', 'pvc' ); ?>"
								>
								<?php
									for ( $i = 0; $i <= 20; $i++ ) {
										?>
                           ...
                           <!-- Box Spread -->
							<select
                                ...
                                data-placeholder="<?php _e( 'Spread Size', 'pvc' ); ?>"
								>
								<?php
									for ( $i = 0; $i <= 20; $i++ ) {
										?>
                           ...
                           <!-- Box Shadow Inset -->
                                <input
                                    ...
                                    checked_label="<?php _e( 'INNER', 'pvc' ); ?>"
                                    unchecked_label="<?php _e( 'OUTER', 'pvc' ); ?>"
                                    type="checkbox"
                                    value="inset"
                           ...
                           <input
                            ...
                            <!-- Preview Button -->
                           <div class="a3rev-ui-settings-preview"><a href="#" class="a3rev-ui-box_shadow-preview-button a3rev-ui-settings-preview-button button submit-button" title="<?php _e( 'Preview your customized box shadow settings', 'pvc'); ?>"><span>&nbsp;</span></a></div>   
                           </div>
                           <div style="clear:both;"></div>
                           </div>
						</td>
				...
				case 'upload':
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
                        	<?php echo $description; ?>
                        	<?php echo $wp_pvc_uploader->upload_input( $name_attribute, $id_attribute, $option_value, $value['default'], esc_html( $value['name'] ), $class, esc_attr( $value['css'] ) , '' );?>
						</td>
					</tr><?php
		...
		<?php do_action( $this->plugin_name . '-' . trim( $form_key ) . '_settings_end' ); ?>
            <p class="submit">
                    <input type="submit" value="<?php _e('Save changes', 'pvc'); ?>" class="button button-primary" name="bt_save_settings" />
                    <input type="submit" name="bt_reset_settings" class="button" value="<?php _e('Reset Settings', 'pvc'); ?>"  />
