...
class WP_PVC_Uploader extends WP_PVC_Admin_UI
{
	...
	public function upload_input ( $name_attribute, $id_attribute = '', $value = '', $default_value = '', $field_name = '', $class = '', $css = '', $description = '', $post_id = 0 ) {
		...
		$output .= '<input id="upload_'.$id_attribute.'" class="a3rev-ui-upload-button a3_upload_button button" type="button" value="'.__( 'Upload', 'pvc' ).'" rel="'.$post_id.'" /> '.$description;
	...
	public function change_button_text( $translation, $original, $domain ) {
	    ...
	    if ( is_admin() && $original === 'Insert into Post' ) {
	    	$translation = __( 'Use this Image', 'pvc' );
			if ( isset( $_REQUEST['title'] ) && $_REQUEST['title'] != '' ) { $translation =__( 'Use as', 'pvc' ).' '.esc_attr( $_REQUEST['title'] ); }
	    }
	...
	public function modify_tabs ( $tabs ) {
		if ( isset( $tabs['gallery'] ) ) { $tabs['gallery'] = str_replace( 'Gallery', __( 'Previously Uploaded', 'pvc' ), $tabs['gallery'] ); }
		return $tabs;
	}
